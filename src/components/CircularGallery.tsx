  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        // Signed distance to a rounded box. Both p and b (half-size) must be
        // expressed in the SAME physical units so a single radius r reads as
        // a true, uniform-radius circle on every corner regardless of the
        // plane's aspect ratio (card is ~700x900, i.e. not square).
        ...
        void main() {
          ...
          // uBorderRadius is authored as a fraction (0-0.5) of the shorter
          // plane edge. Converting into the plane's physical (world) units
          // before running the SDF is what keeps the four corners perfectly
          // symmetric/circular instead of stretching into ovals on the
          // longer axis.
          float shortSide = min(uPlaneSizes.x, uPlaneSizes.y);
          float radiusPx = uBorderRadius * shortSide;
          vec2 p = (vUv - 0.5) * uPlaneSizes;
          vec2 halfSize = uPlaneSizes * 0.5 - vec2(radiusPx);
          float d = roundedBoxSDF(p, halfSize, radiusPx);

          float edgeSmooth = max(shortSide * 0.0035, 0.0008);
    ...
    const applyTexture = (source: HTMLImageElement) => {
    ...
    img.onerror = () => {
      // A cross-origin texture load can fail when the browser reuses a cached
      // non-CORS copy of this URL (TMDB images send no `Vary: Origin`). Retry
      // once with a unique query string to force a fresh, CORS-enabled fetch.
      const retry = new Image();
      retry.crossOrigin = "anonymous";
      retry.onload = () => applyTexture(retry);
      const separator = this.image.includes("?") ? "&" : "?";
      retry.src = `${this.image}${separator}cors=1`;
    };
    img.src = this.image;
  }
...
class App {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    const x = "touches" in e ? e.touches[0].clientX : e.clientX;
    const y = "touches" in e ? e.touches[0].clientY : e.clientY;
    if (Math.abs(x - this.downX) > 6 || Math.abs(y - this.downY) > 6) {
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    this.isDown = false;
    if (!this.pointerMoved) this.handleClick(e);
    this.onCheck();
  }
  ...
  handleClick(e: MouseEvent | TouchEvent) {
    ...
    // Ignore clicks that land outside the gallery (listeners are on window).
    if (
      clientX < rect.left ||
      clientX > rect.right ||
      clientY < rect.top ||
    ...
    const worldX =
      ((clientX - rect.left) / rect.width - 0.5) * this.viewport.width;
    let hit: Media | null = null;
    for (const media of this.medias) {
      if (
        Math.abs(worldX - media.plane.position.x) <=
        media.plane.scale.x / 2
      ) {
        hit = media;
        break;
      }
    ...
    if (!hit) return;
    const total = this.mediasImages.length / 2 || 1;
    const originalIndex = ((hit.index % total) + total) % total;
...
export default function CircularGallery({
  ...
  bend = 3,
  textColor = "#ffffff",
  // 0.12 ≈ the large, symmetric corner rounding seen on the reference card
  // (roughly 12% of the shorter card edge). Bump this up/down to taste.
  borderRadius = 0.12,
  font = "bold 30px Figtree",
  fontUrl,
  scrollSpeed = 2,
